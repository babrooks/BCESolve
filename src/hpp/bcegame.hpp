...
protected:
  ...
  vector<int> numActions; /*!< The number of action profiles for each
                             player. */
  int numStates; /*!< The number of payoff relevant states. */
  vector<int> numTypes; /*!< The number of private types for each
                           player. */
  int numObjectives; /*!< The number of objective functions. Must be
		       >= 2. */

  //! Prior over state and types
  /*! For each state and vector of types, returns the prior
      probability of those occuring. */
  ...
  /*! If a combination of actions and types is dominated, the
      ...
      (given that type) to be zero. */
  bool dominated(int action, int type, int player) const {return false;};
  ...
  /*! Indicates whether or not it is feasible to deviate from the
    action to the alternative dev, when the player's type is type. */
