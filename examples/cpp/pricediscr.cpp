// Ben's file for studying linear programs of the monopoly price
...
int main (int argc, char**argv)
{
  ...
  double prior[numVals];
  double probsum = 0;
  // Uniform prior
  for (valIdx = 0; valIdx < numVals; valIdx++)
    prior[valIdx] = 1.0/numVals;
  // Random prior
  srand(time(NULL));
  for (valIdx = 0; valIdx < numVals; valIdx++)
    {
      prior[valIdx] = rand()%1000;
      probsum += prior[valIdx];
    }
  for (valIdx = 0; valIdx < numVals; valIdx++)
    {
      prior[valIdx] /= probsum;
    }

  cout << "Prior: (";
  for (valIdx=0; valIdx < numVals-1; valIdx++)
    cout << prior[valIdx] << ", ";
  cout << prior[numVals-1] << ")" << endl << endl;

  double priorSum[numVals];
  int monopolyPriceIdx = numVals-1;
  double monopolyPrice = highVal,
    monopolyProfits=highVal * prior[numVals-1], 
    monopolyQuantity = prior[numVals-1], 
    monopolySurplus=0.0;
  priorSum[numVals-1] = prior[numVals-1];
  for (priceIdx = numVals-2, price = highVal-valIncr; 
       priceIdx>=0; 
       priceIdx--, price -= valIncr)
    {
      priorSum[priceIdx] = prior[priceIdx] + priorSum[priceIdx+1];
      
      if (price*priorSum[priceIdx] > monopolyProfits)
	{
	  monopolyPrice = priceIdx;
	  monopolyProfits = price*priorSum[priceIdx];
	  monopolyQuantity = priorSum[priceIdx];
	  monopolySurplus = 0.0;
	  monopolyPriceIdx = priceIdx;
	  for (valIdx = priceIdx, val = price; 
	       valIdx < numVals; 
	       valIdx++, val += valIncr)
	    monopolySurplus += val * prior[valIdx];
	} // if
    } // for

  cout << "Monopoly price: " << monopolyPrice << endl
       << "Monopoly profits: " << monopolyProfits << endl
       << "Monopoly quantity: " << monopolyQuantity << endl
       << "Monopoly total surplus: " << monopolySurplus << endl;
  ...
  try
	...
	con.add(IloRange(env,prior[valIdx],prior[valIdx]));
      ...
      con.add(IloRangeArray(env,numVals*numVals,-IloInfinity,0.0));
